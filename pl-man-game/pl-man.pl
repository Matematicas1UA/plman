...
%%	
:- module('pl-man', [ doAction/1, havingObject/0, havingObject/1, see/3, hear/2, play/2, play/3, play/4, replay/2]).
...
updateEntities:-
	...
	ignore(call_cleanup(PacRule, State, alertState(State))), !,
...
%   BotRule: Rule to control the pacman
%    
%   Initializes the environment setting everything to its
%   initial state.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p_initPacmanRule(BotRule):-
	functor(BR, BotRule, 1), 
	arg(1, BR, init), 
	clause(BR, _),
	ignore(call_cleanup(BR, State, alertState(State))).
p_initPacmanRule(_):- !.
gameInit(MapFile, BotRule):-
	...
	createGameEntity('@', pacman, 1, 1, active, BotRule, 0),
	playerStats(reset),
	loadNewMap(MapFile),
	p_initPacmanRule(BotRule).
...
play(MapFile, BotRule, SleepTime, L_PARAMS):- 
	gameInit(MapFile, BotRule), !,
